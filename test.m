...
%%
scatter3(Tx_pos(1), Tx_pos(2), Tx_pos(3), 100, 'r', '*');
hold on;
scatter3(Rx_pos(1), Rx_pos(2), Rx_pos(3), 100, 'b', '*');
scatter3(mid(1), mid(2), mid(3), 100, 'g', 'filled');
[x_plane, y_plane] = meshgrid(-3:3, -3:3);
% Ax + By + Cz + D = 0;
...
scatter3(Tx_projection(1), Tx_projection(2), Tx_projection(3), 100, 'r', 'filled');
scatter3(Rx_projection(1), Rx_projection(2), Rx_projection(3), 100, 'b', 'filled');
line([Tx_pos(1) mid(1)],[Tx_pos(2) mid(2)],...
    [Tx_pos(3) mid(3)],'LineStyle',':',...
    'color','k','LineWidth',2)
line([Tx_projection(1) mid(1)],[Tx_projection(2) mid(2)],...
    [Tx_projection(3) mid(3)],'LineStyle',':',...
    'color','k','LineWidth',2)
line([Rx_pos(1) mid(1)],[Rx_pos(2) mid(2)],...
    [Rx_pos(3) mid(3)],'LineStyle',':',...
    'color','k','LineWidth',2)
line([Rx_projection(1) mid(1)],[Rx_projection(2) mid(2)],...
    [Rx_projection(3) mid(3)],'LineStyle',':',...
    'color','k','LineWidth',2)
xlabel('X');
ylabel('Y');
zlabel('Z');
legend('Tx\_pos', 'Rx\_pos', 'mid', 'Plane P', 'Tx\_projection', 'Rx\_projection');
title(['\angle (rad) = ', num2str(angle_rad, '%.2f')]);
